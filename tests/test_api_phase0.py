...
async def test_echo_skill_success(async_client: httpx.AsyncClient):
...
async def test_text_manipulation_skill_uppercase_success(async_client: httpx.AsyncClient):
...
async def test_text_manipulation_skill_invalid_operation(async_client: httpx.AsyncClient):
...
async def test_simple_math_skill_add_success(async_client: httpx.AsyncClient):
...
async def test_simple_math_skill_divide_by_zero(async_client: httpx.AsyncClient):
...
async def test_simple_math_skill_missing_number(async_client: httpx.AsyncClient):
...
async def test_process_non_existent_skill(async_client: httpx.AsyncClient):
...
async def test_process_missing_task_type(async_client: httpx.AsyncClient):
...
async def test_process_missing_prompt(async_client: httpx.AsyncClient):
...
async def test_list_all_skills_capabilities(async_client: httpx.AsyncClient):
...
async def test_get_specific_skill_capabilities_success(async_client: httpx.AsyncClient):
    response = await async_client.get("/api/v1/skills/echo/capabilities")
    assert response.status_code == 200
    data = response.json()
    assert data["skill_name"] == "echo"
    assert "A simple skill that echoes back the provided prompt." in data["description"]

async def test_get_capabilities_non_existent_skill(async_client: httpx.AsyncClient):
    response = await async_client.get("/api/v1/skills/non_existent_skill/capabilities")
    assert response.status_code == 404
    data = response.json()
